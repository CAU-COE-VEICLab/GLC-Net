...
class PatchEmbed(nn.Module):
    r""" Result_Seg to Patch Embedding

    Args:
        img_size (int): Result_Seg size.  Default: 224.
...
class GlobalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        x_2d = x.view(B, H, W, C).permute(0,3,1,2).contiguous()  # [B,C,H,W] 
...
if __name__ == '__main__':
    ...
    net = Dy_Seg_Global(
        poolstep=[8,4,2,1],Nq = 5,
        img_size=512,
        window_size=16,
        embed_dim=96,
        n_classes=4, drop_path_rate=0.3,depths=[2,2,18,2] ).cuda()
    # print(net)
